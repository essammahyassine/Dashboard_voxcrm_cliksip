...
<?php
    ...
    else
    {
        ...
        AND camp.id = s.campaign_id and s.positive=1 and camp.id="'.$idcamp.'" and o.agent_id="'.$idag.'" AND DATE_FORMAT( o.agent_start, "%d/%m/%y" )="'.$dt.'" group by s.text';
        ...
        LEFT JOIN campaigns_status s ON o.status = s.num
        WHERE s.text IS NOT NULL 
        AND camp.id = s.campaign_id and s.argued=1 and camp.id="'.$idcamp.'" and o.agent_id="'.$idag.'" AND DATE_FORMAT( o.agent_start, "%d/%m/%y" )="'.$dt.'" group by s.text';
        ...
        LEFT JOIN campaigns_status s ON o.status = s.num
        WHERE s.text IS NOT NULL 
        AND camp.id = s.campaign_id and s.positive=1 and camp.id="'.$idcamp.'" and o.agent_id="'.$idag.'" AND DATE_FORMAT( o.agent_start, "%d/%m/%y" )="'.$dt.'"')->fetchColumn(); 
        ...
        LEFT JOIN campaigns_status s ON o.status = s.num
        WHERE s.text IS NOT NULL 
        AND camp.id = s.campaign_id and s.argued=1 and camp.id="'.$idcamp.'" and o.agent_id="'.$idag.'" AND DATE_FORMAT( o.agent_start, "%d/%m/%y" )="'.$dt.'"')->fetchColumn(); 
        ...
        LEFT JOIN campaigns_status s ON o.status = s.num
        WHERE s.text IS NOT NULL 
        AND camp.id = s.campaign_id and s.text!="" and camp.id="'.$idcamp.'" and o.agent_id="'.$idag.'" AND DATE_FORMAT( o.agent_start, "%d/%m/%y" )="'.$dt.'"';
        ...
        LEFT JOIN campaigns_status s ON o.status = s.num
        WHERE s.text IS NOT NULL 
        AND camp.id = s.campaign_id and s.text in ("Répondeur","Repondeur") and camp.id="'.$idcamp.'" and o.agent_id="'.$idag.'" AND DATE_FORMAT( o.agent_start, "%d/%m/%y" )="'.$dt.'"';
        ...
        LEFT JOIN campaigns_status s ON o.status = s.num
        WHERE s.text IS NOT NULL 
        AND camp.id = s.campaign_id and s.text in ("Répondeur","Repondeur") and camp.id="'.$idcamp.'" and o.agent_id="'.$idag.'" AND DATE_FORMAT( o.agent_start, "%d/%m/%y" )="'.$dt.'" and (o.caller_id like "00337%" or o.caller_id like "00336%")';
        ...
        LEFT JOIN campaigns_status s ON o.status = s.num
        WHERE s.text IS NOT NULL 
        AND camp.id = s.campaign_id and s.text in ("A rappeler","Rappel personnel") and camp.id="'.$idcamp.'" and o.agent_id="'.$idag.'" AND DATE_FORMAT( o.agent_start, "%d/%m/%y" )="'.$dt.'" and c.RecordFileName=""')->fetchColumn(); 
        ...
        $hourcalc = "SELECT c.REFPROSPECT,u.name as HOTESSE, DATE_FORMAT( o.agent_start, '%d/%m/%y' )  as Dateappel,
                                        ...
                                        AND camp.id='".$idcamp."' and o.agent_id='".$idag."' AND DATE_FORMAT( o.agent_start, '%d/%m/%y' )='".$dt."' order by DATE_FORMAT( o.agent_start, '%T' )";
        ...
        <div class="content mt-3">
            ...
            <div class="col-xl-5">
                <div class="card" >
                           <div class="card">
                        <div class="card-header">
                            <strong class="card-title">Statistiques Inbound ( <?=$dt?> )</strong>
                        </div>
                        <div class="card-body">
                            <table class="table table-striped" id="HO">
                              <thead>
    ...
    <script>
        ( function ( $ ) {
            ...
            <script type="text/javascript">
                 ( function ( $ ) {
                        $.ajax({
                            url: "https://192.168.1.4/DashKGL/posag.php?idcamp=<?=$idcamp?>&dt=<?=$dt?>&idag=<?=$idag?>",
                            method: "GET",
            ...
            <script type="text/javascript">
                 ( function ( $ ) {
                        $.ajax({
                            url: "https://192.168.1.4/DashKGL/argag.php?idcamp=<?=$idcamp?>&dt=<?=$dt?>&idag=<?=$idag?>",
                            method: "GET",
                            success: function(data) {
                                console.log(data);
                                var Heure = [];
            ...
            <script type="text/javascript">
                 ( function ( $ ) {
                        $.ajax({
                            url: "https://192.168.1.4/DashKGL/repag.php?idcamp=<?=$idcamp?>&dt=<?=$dt?>&idag=<?=$idag?>",
                            method: "GET",
                            success: function(data) {
                                console.log(data); 
                                var Heure = [];
            ...
            <script type="text/javascript">
                 ( function ( $ ) {
                        $.ajax({
                            url: "https://192.168.1.4/DashKGL/rapag.php?idcamp=<?=$idcamp?>&dt=<?=$dt?>&idag=<?=$idag?>",
                            method: "GET",
                            success: function(data) {
                                console.log(data); 
                                var Heure = [];
